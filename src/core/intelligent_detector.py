...
class IntelligentDetector:
    ...
    def __init__(self, config: Config):
        ...
        self.bypass_modes = {
            ...
            'mtp_talkback': BypassMode(
                name='MTP TalkBack',
                connection_type='mtp',
                success_rate=0.80,
                priority=6,
                description='MTP mode TalkBack exploit',
                best_methods=['tcl_talkback_bypass', 'adb_talkback_chrome', 'emergency_call_exploit']
            ),
            'recovery_wipe': BypassMode(
                name='Recovery Wipe',
                connection_type='recovery',
                success_rate=0.70,
                priority=7,
                description='Recovery mode factory reset/wipe exploit',
                best_methods=['recovery_factory_reset', 'recovery_data_wipe']
            )
        }
    ...
    def switch_device_to_optimal_mode(self, device: DeviceInfo, target_mode: BypassMode) -> bool:
        ...
        try:
            ...
            elif target_mode.connection_type == 'recovery' and device.connection_type == 'mtp':
                return self._switch_mtp_to_recovery(device)
            elif target_mode.connection_type == 'recovery' and device.connection_type == 'adb':
                return self._switch_adb_to_recovery(device)
            else:
    ...
    def _switch_to_download_mode(self, device: DeviceInfo) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to switch to download mode: {e}")
            return False
    
    def _switch_mtp_to_recovery(self, device: DeviceInfo) -> bool:
        """Switch device from MTP to recovery mode"""
        try:
            self.logger.info(f"Attempting to switch {device.serial} from MTP to recovery mode")
            
            # For MTP devices, we need to use ADB commands even if unauthorized
            # Try to reboot to recovery
            result = subprocess.run(
                ['adb', '-s', device.serial, 'reboot', 'recovery'],
                capture_output=True, text=True, timeout=30
            )
            
            if result.returncode == 0:
                self.logger.info(f"Successfully rebooted {device.serial} to recovery mode")
                time.sleep(10)
                return True
            else:
                self.logger.warning(f"Failed to reboot to recovery: {result.stderr}")
                return False
                
        except Exception as e:
            self.logger.error(f"MTP to recovery switch failed: {e}")
            return False
    
    def _switch_adb_to_recovery(self, device: DeviceInfo) -> bool:
        """Switch device from ADB to recovery mode"""
        try:
            self.logger.info(f"Attempting to switch {device.serial} from ADB to recovery mode")
            ...
            self.logger.error(f"ADB to recovery switch failed: {e}")
            return False
